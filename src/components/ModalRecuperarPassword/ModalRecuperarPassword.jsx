...
const ModalRecuperarPassword = () => {
  ...
  const solicitarContrasena = async (rol) => {
    try {
      ...
      navigate("/");
      toast.success(
        "Solicitud enviada. Si no aparece email, revise casilla de spam"
      );
    } catch (error) {
    ...
    <Modal>
      ...
      <ModalBody className="space-y-3">
        <ModalContent>
          ...
          <ModalFooter>
            <Button onClick={() => solicitarContrasena("Administrador")}>
              Como administrador
            </Button>
            <Button onClick={() => solicitarContrasena("Tecnico")}>
              Como técnico
            </Button>
          </ModalFooter>
        </ModalContent>
